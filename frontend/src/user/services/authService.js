...
export const loginUser = async (userData) => {
  ...
  if (data.token && data.employee) {
    ...
    const istMidnight = new Date(utcMidnight.getTime() + istOffset);
...
export const getTicketOptionsForDepartment = async (departmentName) => {
  try {
    const token = localStorage.getItem("token");
    if (!token) {
      throw new Error("No authentication token found");
    ...
    const response = await fetch(`${API_URL}/ticket-options?${params.toString()}`, {
      method: "GET",
      headers: {
        Authorization: `Bearer ${token}`,
      },
    ...
    const data = await response.json();
    if (!response.ok) {
      throw new Error(data.message || "Failed to fetch ticket options");
    }

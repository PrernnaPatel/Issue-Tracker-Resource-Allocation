...
export const DeptSocketProvider = ({ children }) => {
  ...
  useEffect(() => {
    ...
    socket.on("new-ticket", (data) => {
      ...
      if (!deptAdmin?.isNetworkEngineer) {
        const locations = Array.isArray(deptAdmin?.locations) ? deptAdmin.locations : [];
        if (locations.length > 0 && data?.buildingId) {
          const allowedBuildingIds = locations
            .map((loc) =>
          ...
          if (!allowedBuildingIds.includes(String(data.buildingId))) {
            return;
          }
        }
      }
